...
class BatchDetectLanguageTextClassificationController extends Controller
{
    public function DetectLanguageTextClassification(Request $request)
    {
        ...
        foreach ($res['documents'][0]['textClassification'] as &$language) {
            unset($language['score']);
        }
        ...
        return response()->json([
            ...
            'request_id' => $genKey,
            'response' => [
                $res['documents'][0]['textClassification']
            ]
        ], 200);
    }
}
